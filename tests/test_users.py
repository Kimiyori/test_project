...
async def test_user_accounts(app, get_token, create_account):
    ...
    _, response = await app.asgi_client.get("/users/accounts", headers=headers)
...
async def test_user_transactions(app, get_token, create_transactions):
    headers = {"Authorization": f"Bearer {get_token}"}
    _, response = await app.asgi_client.get("/users/transactions", headers=headers)
    assert response.status_code == 200
...
async def test_disable_user(app, dao_session, get_admin_token):
    ...
    content = {"id": 2, "status": "disable"}
    headers = {"Authorization": f"Bearer {get_admin_token}"}
    _, response = await app.asgi_client.patch(
        "/admin/users/status/", headers=headers, content=json.dumps(content)
    )
    assert response.status_code == 200
